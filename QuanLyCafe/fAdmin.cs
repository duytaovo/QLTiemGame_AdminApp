...
namespace QuanLyCafe
{
    public partial class fAdmin : Form
    {
        ...
        void AddFoodBinding()
        {
            //Food food = (Food)dtgv_food.CurrentRow.DataBoundItem;
            //txt_foodName.Text = food.Name;
            //txt_foodID.Text = food.Id.ToString();
            //float price = food.Price;
            //nm_foodPrice.Value = (decimal)price;
        ...
        {
            //int a = (int)dtgv_food.Rows[e.RowIndex].Cells[e.ColumnIndex].Value;
            //Food food = cb_Food.SelectedItem as Food;
            //Food food = (Food)dtgv_food.CurrentRow.DataBoundItem;
            //ptB_image.Image = Image.FromFile(food.Link_image);
            int index = e.RowIndex;
            DataGridViewRow selectedRow = dtgv_food.Rows[index];
            int a = (int)selectedRow.Cells[0].Value;
        ...
        private void btn_addFood_Click(object sender, EventArgs e)
        {
            ...
            int idFood = fTableManager.links_image.Count();
            //var result = MessageBox.Show("Bạn có muốn thêm hình ảnh của món ăn không? ", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.None);

            string link_image = "";
            Add_foods_image new_form = new Add_foods_image();
            this.Hide();
            new_form.ShowDialog();
            this.Show();
            link_image = Add_foods_image.Link_Image;
            if (FoodDAO.Instance.InsertFood(name, categogyID, price, link_image))
            {
